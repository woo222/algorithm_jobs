'''
# 문제 - 평면에 색깔이 서로 다른 직사각형 모양의 색종이 N장이 하나씩 차례로 놓여진다. 이 때 색종이가 비스듬하게 놓이는 경우는 없다. 
...
# 입력 - 입력의 첫 번째 줄에는 색종이의 장수를 나타내는 정수 N (1 ≤ N ≤ 100)이 주어진다. 이어서 N장의 색종이에 관한 입력이 각 색종이마다 한 줄씩 차례로 주어진다. 
...
paper_num = int(input())
paper = [[None for _ in range(101)]for __ in range(101)]

for i in range(paper_num):
  x, y, w, h = map(int, input().split())
  
  for j in range(y, y+h): # 높이 = 행
    for k in range(x, x+w): # 넓이 = 열
      paper[j][k] = i + 1
